dnl This code was taken from GStreamer repository
dnl Macros for checking gcov support
dnl Usage: ENS_GCOV
...
[
  ...
  if test x$enable_gcov = xyes ; then
    ...
    ENS_GCOV_ENABLED=yes
    AC_DEFINE_UNQUOTED(ENS_GCOV_ENABLED, 1,
      [Defined if gcov is enabled to force a rebuild due to config.h changing])
    dnl if gcov is used, we do not want default -O2 CFLAGS
    if test "x$ENS_GCOV_ENABLED" = "xyes"
    then
      CFLAGS="$CFLAGS -O0"
      AC_SUBST(CFLAGS)
  ...
  AM_CONDITIONAL(ENS_GCOV_ENABLED, test x$enable_gcov = xyes)
])
